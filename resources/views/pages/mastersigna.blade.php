...
<div class="bg-white p-6 rounded shadow-md">
    <h2 class="text-xl font-bold mb-4 text-blue-700">Daftar Signa</h2>

    <!-- Search Form -->
    <form method="GET" class="mb-4">
        <input type="text" name="search" value="{{ $keyword ?? '' }}"
            class="border px-4 py-2 rounded w-64" placeholder="Cari kode signa">
        <button type="submit"
            ...
            <tbody>
                @forelse ($data as $index => $item)
                    <tr class="text-sm hover:bg-gray-100">
                        ...
                        <td class="px-4 py-2 border">{{ $item->signa_kode }}</td>
                        <td class="px-4 py-2 border">{{ $item->signa_nama }}</td>
                    </tr>
                @empty
                    <tr>
